...
describe('Image helpers', function () {
  ...
  it('Should skip processing if the source image is okay', async function () {
    ...
    await processImage({ source: input, destination: imageDestJPG, newSize: thumbnailSize, keepOriginal: true })
  ...
  it('Should strip exif for a jpg file that can not be copied', async function () {
    ...
    await processImage({ source: input, destination: imageDestJPG, newSize: { width: 100, height: 100 }, keepOriginal: true })
    await checkBuffers(input, imageDestJPG, false)
  ...
  it('Should strip exif for png', async function () {
    ...
    await processImage({ source: input, destination: imageDestPNG, newSize: thumbnailSize, keepOriginal: true })
