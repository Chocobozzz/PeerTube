...
export class VideoWatchComponent implements OnInit, OnDestroy {
  @ViewChild("videoWatchPlaylist", { static: true })
  videoWatchPlaylist: VideoWatchPlaylistComponent;
  ...
  isUserOwner() {
    return (
      this.video.isLocal === true &&
      this.video.account.name === this.user?.username
    );
  }
  ...
  private loadRouteQuery() {
    this.queryParamsSub = this.route.queryParams.subscribe((queryParams) => {
      ...
      if (isNaN(this.playlistPosition)) {
        logger.error(
          `playlistPosition query param '${positionParam}' was parsed as NaN, defaulting to 1.`
        );
  ...
  private loadVideo(options: {
    ...
    const videoAndLiveObs: Observable<{
      video: VideoDetails;
      live?: LiveVideo;
      ...
      switchMap((video) => {
        ...
        return this.liveVideoService
          .getVideoLive(video.uuid)
          .pipe(map((live) => ({ live, video })));
      }),

      switchMap(({ video, live }) => {
        if (!videoRequiresFileToken(video))
          return of({ video, live, videoFileToken: undefined });

        return this.videoFileTokenService
          .getVideoFileToken({ videoUUID: video.uuid, videoPassword })
    ...
    forkJoin([
      ...
      error: async (err) => {
        if (
          err.body.code === ServerErrorCode.VIDEO_REQUIRES_PASSWORD ||
          err.body.code === ServerErrorCode.INCORRECT_VIDEO_PASSWORD
        ) {
          const { confirmed, password } = await this.handleVideoPasswordError(
            err
          );
  ...
  private loadPlaylist(playlistId: string) {
    ...
    this.playlistService.getVideoPlaylist(playlistId).subscribe({
      next: (playlist) => {
  ...
  private async handleRequestError(err: any) {
    ...
    if (
      errorBody?.code === ServerErrorCode.DOES_NOT_RESPECT_FOLLOW_CONSTRAINTS &&
      errorBody.originUrl
    ) {
  ...
  private handleVideoPasswordError(err: any) {
    ...
    return this.confirmService.confirmWithPassword({
      ...
      errorMessage: isIncorrectPassword
        ? $localize`Incorrect password, please enter a correct password`
        : "",
    });
  }
  ...
  private async onVideoFetched(options: {
    ...
    this.loadPlayer({ loggedInOrAnonymousUser, forceAutoplay }).catch((err) =>
      logger.error("Cannot build the player", err)
    );
  ...
  private async loadPlayer(options: {
    ...
    if (
      videoState === VideoState.LIVE_ENDED ||
      videoState === VideoState.WAITING_FOR_LIVE
    ) {
  ...
  private isAutoplay() {
    ...
    throw new Error(
      "Cannot guess autoplay because user and anonymousUser are not defined"
    );
  }

  private isAutoPlayNext() {
    return this.user?.autoPlayNextVideo || this.anonymousUser.autoPlayNextVideo;
  }
  ...
  private buildPeerTubePlayerConstructorOptions(options: {
    ...
    return {
      ...
      metricsUrl: this.serverConfig.openTelemetry.metrics.enabled
        ...
        : null,
      metricsInterval:
        this.serverConfig.openTelemetry.metrics.playbackStatsInterval,
  ...
  private buildPeerTubePlayerLoadOptions(options: {
    ...
    const getStartTime = () => {
      ...
      const byHistory =
        video.userHistory &&
        (!this.playlist || urlOptions.resume !== undefined);
    ...
    return {
      ...
      p2pEnabled: isP2PEnabled(
        video,
        this.serverConfig,
        loggedInOrAnonymousUser.p2pEnabled
      ),
      ...
      videoViewUrl:
        video.privacy.id !== VideoPrivacy.PRIVATE
  ...
  private async subscribeToLiveEventsIfNeeded(
    oldVideo: VideoDetails,
  ...
  private handleLiveViewsChange(newViewers: number) {
    if (!this.video) {
      logger.error(
        "Cannot update video live views because video is no defined."
      );
      return;
    }
    ...
    if (!video.isLive) {
      this.hotkeys = this.hotkeys.concat([
        ...
        new Hotkey(
          "0-9",
          (e) => e,
          $localize`Skip to a percentage of the video: 0 is 0% and 9 is 90%`
        ),
        ...
        new Hotkey(
          ",",
          (e) => e,
          $localize`Navigate in the video to the previous frame`
        ),
        new Hotkey(
          ".",
          (e) => e,
          $localize`Navigate in the video to the next frame`
        ),
      ]);
    }
    ...
    if (this.isUserLoggedIn()) {
      this.hotkeys = this.hotkeys.concat([
        new Hotkey(
          "shift+s",
          () => {
            if (this.subscribeButton.isSubscribedToAll())
