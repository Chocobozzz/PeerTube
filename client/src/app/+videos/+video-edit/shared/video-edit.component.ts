...
import {
  VIDEO_CHAPTERS_ARRAY_VALIDATOR,
  VIDEO_CHAPTER_TITLE_VALIDATOR,
} from "@app/shared/form-validators/video-chapter-validators";
import {
...
import {
  VideoCaptionEdit,
  VideoCaptionWithPathEdit,
} from "@app/shared/shared-main/video-caption/video-caption-edit.model";
...
import {
  SelectChannelItem,
  SelectOptionsItem,
} from "src/types/select-options-item.model";
...
export class VideoEditComponent implements OnInit, OnDestroy {
  ...
  @ViewChild("videoCaptionAddModal", { static: true })
  videoCaptionAddModal: VideoCaptionAddModalComponent;
  ...
  updateForm() {
    ...
    this.form.addControl(
      "chapters",
      new FormArray([], VIDEO_CHAPTERS_ARRAY_VALIDATOR.VALIDATORS)
    );
  ...
  ngOnInit() {
    ...
    this.pluginService
      .ensurePluginsAreLoaded("video-edit")
    ...
    forkJoin([
      ...
      .subscribe(({ about, languages }) => {
        this.videoLanguages = [
          ...
          ...languages.filter(
            (l) => !about.instance.languages.includes(l.id) && l.id !== "zxx"
          ),
        ];
      });

    this.serverService.getVideoPrivacies().subscribe((privacies) => {
      const videoPrivacies =
        this.videoService.explainedPrivacyLabels(privacies).videoPrivacies;
      this.videoPrivacies = videoPrivacies;
      this.replayPrivacies = videoPrivacies.filter(
        (privacy) => privacy.id !== VideoPrivacy.PASSWORD_PROTECTED
      );

      // Can't schedule publication if private privacy is not available (could be deleted by a plugin)
      const hasPrivatePrivacy = this.videoPrivacies.some(
        (p) => p.id === VideoPrivacy.PRIVATE
      );
    ...
    this.hooks.runAction("action:video-edit.init", "video-edit", {
      type: this.type,
      updateForm: updateFormForPlugins,
    });

    this.form.valueChanges.subscribe(() => {
      this.hooks.runAction("action:video-edit.form.updated", "video-edit", {
        type: this.type,
  ...
  onCaptionEdited(caption: VideoCaptionEdit) {
    const existingCaption = this.videoCaptions.find(
      (c) => c.language.id === caption.language.id
    );
  ...
  deleteCaption(caption: VideoCaptionEdit) {
    ...
    if (
      caption.action &&
      this.initialVideoCaptions.includes(caption.language.id)
    ) {
  ...
  openEditCaptionModal (videoCaption: VideoCaptionWithPathEdit) {
    ...
    modalRef.componentInstance.captionEdited.subscribe(
      this.onCaptionEdited.bind(this)
    );
  }
  ...
  private async updatePluginFields() {
    this.pluginFields = this.pluginService.getRegisteredVideoFormFields(
      this.type
    );
    ...
    for (const setting of this.pluginFields) {
      await this.pluginService.translateSetting(
        setting.pluginInfo.plugin.npmName,
        setting.commonOptions
      );
      ...
      const validator = async (control: AbstractControl) => {
        ...
        const error = await setting.commonOptions.error({
          formValues: this.form.value,
          value: control.value,
        });

        return error?.error
          ? { [setting.commonOptions.name]: error.text }
          : null;
      };
    ...
    this.form.valueChanges.subscribe(() =>
      this.formValidatorService.updateTreeValidity(this.pluginDataFormGroup)
    );
  }
  ...
  private trackPrivacyChange() {
    ...
    this.form.controls["privacy"].valueChanges
      ...
      .subscribe((newPrivacyId) => {
        console.log("newPrivacyId", newPrivacyId);

        this.schedulePublicationSelected =
          newPrivacyId === VideoPrivacy.SCHEDULED;
        ...
        this.passwordProtectionSelected =
          newPrivacyId === VideoPrivacy.PASSWORD_PROTECTED;
  ...
  private trackChannelChange() {
    ...
    this.form.controls["channelId"].valueChanges
      .pipe(map((res) => parseInt(res.toString(), 10)))
      .subscribe((newChannelId) => {
        const oldChannelId = parseInt(this.form.value["channelId"], 10);

        // Not initialized yet
        if (isNaN(newChannelId)) return;
        const newChannel = this.userVideoChannels.find(
          (c) => c.id === newChannelId
        );
        ...
        setTimeout(() => {
          ...
          const oldChannel = this.userVideoChannels.find(
            (c) => c.id === oldChannelId
          );
