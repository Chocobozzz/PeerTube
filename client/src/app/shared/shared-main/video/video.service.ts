...
export class VideoService {
  static BASE_VIDEO_DOWNLOAD_URL =
    environment.originServerUrl + "/download/videos/generate";
  static BASE_VIDEO_URL = environment.apiUrl + "/api/v1/videos";
  static BASE_FEEDS_URL = environment.apiUrl + "/feeds/videos.";
  static PODCAST_FEEDS_URL = environment.apiUrl + "/feeds/podcast/videos.xml";
  static BASE_SUBSCRIPTION_FEEDS_URL =
    environment.apiUrl + "/feeds/subscriptions.";
  ...
  getVideo(options: {
    ...
    const headers = VideoPasswordService.buildVideoPasswordHeader(
    ...
    return this.serverService.getServerLocale().pipe(
      switchMap((translations) => {
        return this.authHttp
          .get<VideoDetailsServerModel>(
            `${VideoService.BASE_VIDEO_URL}/${options.videoId}`,
            { headers }
          )
          .pipe(map((videoHash) => ({ videoHash, translations })));
      }),
      map(
        ({ videoHash, translations }) =>
  ...
  updateVideo(video: VideoEdit) {
    ...
    return this.authHttp
      .put(`${VideoService.BASE_VIDEO_URL}/${video.id}`, data)
  ...
  uploadVideo(video: FormData) {
    const req = new HttpRequest(
      "POST",
      `${VideoService.BASE_VIDEO_URL}/upload`,
      video,
      { reportProgress: true }
    );
  ...
  getMyVideos(options: {
    ...
    return this.authHttp
      .get<ResultList<Video>>(UserService.BASE_USERS_URL + "me/videos", {
        params,
      })
      .pipe(
  ...
  getAccountVideos(
    ...
    return this.authHttp
      .get<ResultList<Video>>(
        AccountService.BASE_ACCOUNT_URL + account.nameWithHost + "/videos",
        { params }
      )
      .pipe(
        switchMap((res) => this.extractVideos(res)),
  ...
  getVideoFeedUrls(
    ...
    let params = this.restService.addRestGetParams(
      new HttpParams(),
      undefined,
      sort
    );
  ...
  getVideoSubscriptionFeedUrls(accountId: number, feedToken: string) {
    ...
    return this.buildBaseFeedUrls(
      params,
      VideoService.BASE_SUBSCRIPTION_FEEDS_URL
    );
  }
  ...
  removeVideo(idArg: number | number[]) {
    ...
    return from(ids).pipe(
      concatMap((id) =>
        this.authHttp.delete(`${VideoService.BASE_VIDEO_URL}/${id}`)
      ),
      toArray(),
      catchError((err) => this.restExtractor.handleError(err))
    );
    ...
    return from(videoIds).pipe(
      concatMap((id) =>
        this.authHttp.delete(
          VideoService.BASE_VIDEO_URL + "/" + id + "/" + type
        )
      ),
      toArray(),
      catchError((err) => this.restExtractor.handleError(err))
  ...
  removeFile(
    ...
    return this.authHttp
      .delete(
        VideoService.BASE_VIDEO_URL + "/" + videoId + "/" + type + "/" + fileId
      )
      .pipe(catchError((err) => this.restExtractor.handleError(err)));
  }
  ...
  removeSourceFile(videoId: number | string) {
    return this.authHttp
      .delete(VideoService.BASE_VIDEO_URL + "/" + videoId + "/source/file")
      .pipe(catchError((err) => this.restExtractor.handleError(err)));
  }
  ...
  runTranscoding(options: {
    ...
    const body: VideoTranscodingCreate = {
    ...
    return from(videos).pipe(
      concatMap((video) => {
        return this.authHttp
          .post(
            VideoService.BASE_VIDEO_URL + "/" + video.uuid + "/transcoding",
            body
          )
          .pipe(
            catchError((err) => {
              if (
                ...
                const message =
                  $localize`PeerTube considers video "${video.name}" is already being transcoded.` +
  ...
  generateDownloadUrl(options: {
    ...
    if (files.length === 0)
      throw new Error("Cannot generate download URL without files");
  ...
  getStoryboards(videoId: string | number, videoPassword: string) {
    const headers =
      VideoPasswordService.buildVideoPasswordHeader(videoPassword);

    return this.authHttp
      .get<{ storyboards: Storyboard[] }>(
        VideoService.BASE_VIDEO_URL + "/" + videoId + "/storyboards",
        { headers }
      )
      .pipe(
  ...
  explainedPrivacyLabels(
    serverPrivacies: VideoConstant<VideoPrivacyType>[],
    defaultPrivacyId: VideoPrivacyType = VideoPrivacy.PUBLIC
  ) {
    const descriptions = {
      ...
      [VideoPrivacy.SCHEDULED]: $localize`Hide the video until a specific date`,
    };
    ...
    return {
      videoPrivacies,
      defaultPrivacyId:
        serverPrivacies.find((p) => p.id === defaultPrivacyId)?.id ||
        serverPrivacies[0].id,
    };
  }

  getHighestAvailablePrivacy(
    serverPrivacies: VideoConstant<VideoPrivacyType>[]
  ) {
  ...
  nsfwPolicyToParam(nsfwPolicy: NSFWPolicyType): BooleanBothQuery {
    return nsfwPolicy === "do_not_list" ? "false" : "both";
  }
  ...
  buildCommonVideosParams(options: CommonVideoParams & { params: HttpParams }) {
    ...
    let newParams = this.restService.addRestGetParams(
      params,
      pagination,
