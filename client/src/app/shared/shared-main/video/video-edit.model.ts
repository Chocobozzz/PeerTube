...
export class VideoEdit implements VideoUpdate {
  ...
  constructor(video?: VideoDetails, videoPassword?: VideoPassword) {
    ...
    if (video.thumbnailPath)
      this.thumbnailUrl = getAbsoluteAPIUrl() + video.thumbnailPath;
    if (video.previewPath)
      this.previewUrl = getAbsoluteAPIUrl() + video.previewPath;
  ...
  toFormPatch() {
    ...
    if (this.scheduleUpdate) {
      Object.assign(json, {
        ...
        schedulePublicationAt: new Date(
          this.scheduleUpdate.updateAt.toString()
        ),
      });
    }
