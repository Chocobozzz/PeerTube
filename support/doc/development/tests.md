...
```bash
sudo docker run -p 9444:8000 -e S3BACKEND=mem -e REMOTE_MANAGEMENT_DISABLE=1 \
 ...
 * `ENABLE_OBJECT_STORAGE_TESTS=true`: enable object storage tests (needs `ghcr.io/scality/cloudserver` container first)
