...
export class VideoViewModel extends SequelizeModel<VideoViewModel> {
  ...
  static async getTimeserieStats(options: {
    ...
    SELECT
      "intervals"."startDate" AS date, COALESCE("videoView"."downloads", 0) AS value
    FROM
      "intervals"
      LEFT JOIN "videoView" ON "videoView"."videoId" = :videoId
        AND
          "videoView"."startDate" <= "intervals"."endDate"
        AND
          "videoView"."startDate" >= "intervals"."startDate"
    ORDER BY
      "intervals"."startDate"
    `
    ...
    const rows = await VideoViewModel.sequelize.query<any>(query, queryOptions)
