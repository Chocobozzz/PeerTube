...
export const PEERTUBE_VERSION: string = readJsonSync(
...
export const SCHEDULER_INTERVALS_MS = {
  RUNNER_JOB_WATCH_DOG: Math.min(
    CONFIG.REMOTE_RUNNERS.STALLED_JOBS.VOD,
    CONFIG.REMOTE_RUNNERS.STALLED_JOBS.LIVE
  ),
  ...
  CHANNEL_SYNC_CHECK_INTERVAL:
    CONFIG.IMPORT.VIDEO_CHANNEL_SYNCHRONIZATION.CHECK_INTERVAL,
};
...
MIMETYPES.VIDEO_CAPTIONS.EXT_MIMETYPE = invert(
...
export const ACTOR_IMAGES_SIZE: {
  [key in ActorImageType_Type]: { width: number; height: number }[];
} = {
...
export const RESUMABLE_UPLOAD_SESSION_LIFETIME =
  SCHEDULER_INTERVALS_MS.REMOVE_DANGLING_RESUMABLE_UPLOADS;
...
export const ACCEPT_HEADERS = ["html", "application/json"].concat(
  ACTIVITY_PUB.POTENTIAL_ACCEPT_HEADERS
);
export const OTP = {
...
export const ASSETS_PATH = {
  DEFAULT_AUDIO_BACKGROUND: join(
    root(),
    "dist",
    "core",
    "assets",
    "default-audio-background.jpg"
  ),
  DEFAULT_LIVE_BACKGROUND: join(
    root(),
    "dist",
    "core",
    "assets",
    "default-live-background.jpg"
  ),
};
...
export const PLUGIN_GLOBAL_CSS_PATH = join(
...
if (process.env.PRODUCTION_CONSTANTS !== "true") {
  ...
  if (isTestInstance()) {
    ...
    VIDEO_LIVE.CLEANUP_DELAY =
      getIntEnv("PEERTUBE_TEST_CONSTANTS_VIDEO_LIVE_CLEANUP_DELAY") ?? 5000;
...
export async function buildLanguages() {
  ...
  iso6393
    .filter((l) => {
      return (
        (l.iso6391 !== undefined && l.type === "living") ||
...
function updateWebserverUrls() {
  WEBSERVER.URL = sanitizeUrl(
    CONFIG.WEBSERVER.SCHEME +
      "://" +
      CONFIG.WEBSERVER.HOSTNAME +
      ":" +
      CONFIG.WEBSERVER.PORT
  );
  WEBSERVER.HOST = sanitizeHost(
  ...
  const rtmpHostname =
    CONFIG.LIVE.RTMP.PUBLIC_HOSTNAME || CONFIG.WEBSERVER.HOSTNAME;
  const rtmpsHostname =
    CONFIG.LIVE.RTMPS.PUBLIC_HOSTNAME || CONFIG.WEBSERVER.HOSTNAME;

  WEBSERVER.RTMP_URL = "rtmp://" + rtmpHostname + ":" + CONFIG.LIVE.RTMP.PORT;
  WEBSERVER.RTMPS_URL =
    "rtmps://" + rtmpsHostname + ":" + CONFIG.LIVE.RTMPS.PORT;

  WEBSERVER.RTMP_BASE_LIVE_URL =
    WEBSERVER.RTMP_URL + "/" + VIDEO_LIVE.RTMP.BASE_PATH;
  WEBSERVER.RTMPS_BASE_LIVE_URL =
    WEBSERVER.RTMPS_URL + "/" + VIDEO_LIVE.RTMP.BASE_PATH;
}

function updateWebserverConfig() {
  MIMETYPES.VIDEO.MIMETYPE_EXT = buildVideoMimetypeExt();
  MIMETYPES.VIDEO.MIMETYPES_REGEX = buildMimetypesRegex(
    MIMETYPES.VIDEO.MIMETYPE_EXT
  );

  MIMETYPES.VIDEO.EXT_MIMETYPE = buildVideoExtMimetype(
    MIMETYPES.VIDEO.MIMETYPE_EXT
  );
