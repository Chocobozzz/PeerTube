...
const logger = createLogger('ap', 'create')

export function sendCreateVideo (video: MVideoAP, transaction: Transaction) {
...
export function sendCreateCacheFile (
  byActor: MActorLight,
...
export function sendCreateWatchAction (stats: MLocalVideoViewerWithWatchSections, transaction: Transaction) {
  return logger.withContext([ stats.uuid ], async () => {
    logger.info('Creating job to send create watch action %s.', stats.url)
...
export function sendCreateVideoPlaylist (playlist: MVideoPlaylistFull, transaction: Transaction) {
  if (playlist.privacy === VideoPlaylistPrivacy.PRIVATE) return undefined

  return logger.withContext([ playlist.uuid ], async () => {
...
export async function sendCreateVideoCommentIfNeeded (comment: MCommentOwnerVideoReply, transaction: Transaction) {
  return logger.withContext([ comment.url ], async () => {
