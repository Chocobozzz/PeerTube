...
export async function doesVideoExist(
  id: number | string,
  res: Response,
  fetchType: VideoLoadType = "all"
) {
...
export async function doesVideoFileOfVideoExist(
  id: number,
...
export async function doesVideoChannelOfAccountExist(
  channelId: number,
  user: MUserAccountId,
  res: Response
) {
  const videoChannel = await VideoChannelModel.loadAndPopulateAccount(
    channelId
  );
...
export async function checkCanSeeVideo(options: {
  ...
  if (
    ...
    video.privacy === VideoPrivacy.SCHEDULED
  ) {
    return true;
  }

  throw new Error(
    "Unknown video privacy when checking video right " + video.url
  );
}
...
export async function checkCanSeePasswordProtectedVideo(options: {
  ...
  if (!exists(videoPassword)) {
    ...
    if (req.header("authorization")) {
      await authenticatePromise({
        ...
        res,
        errorMessage,
        errorStatus: HttpStatusCode.FORBIDDEN_403,
        errorType,
      });
      const user = res.locals.oauth?.token.User;
  ...
  if (
    await VideoPasswordModel.isACorrectPassword({
      videoId: video.id,
...
export async function getVideoWithRights(
  video: MVideoWithRights
): Promise<MVideoWithRights> {
...
export async function checkCanAccessVideoStaticFiles(options: {
  ...
  if (
    ...
    exists(req.header("x-peertube-video-password"))
  ) {
...
function assignVideoTokenIfNeeded(
  req: Request,
  res: Response,
  video: MVideoUUID
) {
  ...
  if (
    videoFileToken &&
    VideoTokensManager.Instance.hasToken({
      token: videoFileToken,
      videoUUID: video.uuid,
    })
  ) {
    const user = VideoTokensManager.Instance.getUserFromToken({
      token: videoFileToken,
    });
...
export function checkUserCanManageVideo(
  user: MUser,
...
export async function checkUserQuota(
  ...
  if (
    (await isUserQuotaValid({ userId: user.id, uploadSize: videoFileSize })) ===
    false
  ) {
    res.fail({
